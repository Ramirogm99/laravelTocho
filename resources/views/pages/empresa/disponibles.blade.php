...
@section('content_header')
    ...
    <h1>Vallas Disponibles entre : {{ date('d/m/Y', strtotime($f_inicio)) }} -- {{ date('d/m/Y', strtotime($f_fin)) }}</h1>
@stop
...
@section('content')
    <div class="form-check">
        <form action="@if (Auth::user()->auth_level >= 3){{ url('mail/') }} @else {{ url('mail/reserva')}} @endif " method="post">
            @csrf
            <input type="hidden" value="{{$f_inicio}}" name="f_inicio">
            <input type="hidden" value="{{$f_fin}}" name="f_fin">
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                                ...
                                <div class="col-12">
                                    <div class="card card-primary">
                                        <div class="card-footer mx-auto">
                                            ...
                                            <button type="submit" class="btn btn-primary " id="maquetar" disabled>Maquetar
                                                correo</button>
                                            @else 
                                            <button type="submit" class="btn btn-primary " id="maquetar" disabled>Pedir reserva</button>
                                            @endif
        ...
        checks.forEach(check => {
            check.addEventListener('change', function() {
                if (check.checked == true) {
                    maquetar.disabled = false;
                } else {
                    maquetar.disabled = true;
                }
            });
        });
    </script>
