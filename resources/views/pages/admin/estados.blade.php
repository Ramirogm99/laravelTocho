                                            ...
                                            @if ($estado->borrado != 1)
                                                <tr>
                                                    ...
                                                    <td style="text-align: center; vertical-align: middle;"><input
                                                            type="color" value="{{ $estado->color }}" disabled> </input>
                                                    </td>
                                                    <td style="text-align: center; vertical-align: middle;">
                                                        @if ($estado->bloqueado == 0)
                                                            <p
                                                                style="background-color: lightgreen; margin-top: 1rem; margin-bottom: 1rem">
                                                                Activo</p>
                                                        @elseif ($estado->bloqueado == 1)
                                                            <p
                                                                style="background-color:lightcoral; margin-top: 1rem; margin-bottom: 1rem">
                                                                Deshabilitado</p>
                                                        @endif
                                                    </td>
                                                    <td style="text-align: center; vertical-align: middle;">
                                                        {{-- BOTONES DE ACCION --}}
                                                        <a href='{{ url('') }}/estados/show/{{ $estado->id }}'
                                                            class='btn btn-sm btn-dark'><i class="fas fa-eye"></i></a>

                                                        <a href="{{ url('') }}/estados/edit/{{ $estado->id }}"
                                                        ...
                                                        <button class="btn btn-sm btn-danger delete-button"
                                                            ...
                                                            data-val="{{ $estado->id }}"><i class="fas fa-trash-alt"></i>
                                                    </td>
                                                </tr>
                                            @endif
                                        @endforeach
        ...
        @if (session()->has('succ1'))
            <script>
                Swal.fire({
                    icon: 'success',
                    title: 'Correcto',
                    text: 'El estado ha sido creado con exito',
                    timer: 2000,
        ...
        @if (session()->has('succ2'))
            <script>
                Swal.fire({
                    icon: 'success',
                    title: 'Correcto',
                    text: 'El estado ha sido actualizado con exito',
                    timer: 2000,
                    showConfirmButton: false,
                })
            </script>
        ...
        @if (session()->has('succ3'))
            <script>
                Swal.fire({
                    icon: 'success',
                    title: 'Correcto',
                    text: 'El estado ha sido eliminado con exito',
                    timer: 2000,
                    showConfirmButton: false,
                })
            </script>
        ...
        @if (session()->has('error1'))
            <script>
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'Hubo un problema al crear el estado',
                    timer: 2000,
                    showConfirmButton: false,
                })
            </script>
        ...
        @if (session()->has('error3'))
            <script>
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'Hubo un problema al eliminar el estado',
                    timer: 2000,
                    showConfirmButton: false,
                })
            </script>
        ...
        <script>
            $(document).ready(function() {
                $('#tablaestados').DataTable({
                    ...
                    buttons: [
                        // 'copy', 'csv', 'excel', 'pdf', 'print'
                        {
                            ...
                            action: () => {
                                window.location.href = "{{ url('/estados/estadoform') }}";
                            }
                        }
                    ]
                });
                ...
                $(".delete-button").click(function() {
                    ...
                    $('#modalmessage').find("a").first().attr("href", "{{ url('') }}/estados/delete/" +
                        id);
